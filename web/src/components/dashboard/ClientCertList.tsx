...
import { api, type Certificate, type CreateClientCertRequest } from "@/lib/api";
...
export function ClientCertList() {
  ...
  const [formData, setFormData] = useState<CreateClientCertRequest>({
    commonName: "",
    organization: "",
    dnsNames: [],
    ipAddresses: [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.createClientCert(req);
      fetchData();
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Create Client Certificate</CardTitle>
        </CardHeader>
        <CardContent>
